...
function findLargestAreaWithinBounds(points: Point[]): number {
    ...
    for (const pointA of points) {
        for (const pointB of points) {
            ...
            const isWithinBounds = isRectangleWithinPoints(
                [pointA, pointB],
                points
            );
...
function isRectangleWithinPoints(
    corners: [Point, Point],
    points: Point[]
): boolean {
    // if (
    //     !corners.every(
    //         (x) => (x.x === 9 && x.y === 5) || (x.x === 2 && x.y === 3)
    //     )
    // )
    //     return false;
    ...
    // console.log({ lowerXBound, upperXBound });
    // console.log({ lowerYBound, upperYBound });

    const isWithinBounds = isRectangleWithinBounds(corners, bounds);
    const isEveryPointOutsideOfRectangle = points.every((point) => {
        ...
        // console.log({ point, isBetweenXBounds, isBetweenYBounds });

        return !(isBetweenXBounds && isBetweenYBounds);
    });

    // console.log({ isWithinBounds, isEveryPointOutsideOfRectangle });

    return isEveryPointOutsideOfRectangle && isWithinBounds;
